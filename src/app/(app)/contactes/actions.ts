...
async function upsertContact(pool: Pool, f: SyncFields) {
...
export async function syncBandPeopleToContacts(people: { name: string; role: string; phone?: string; email?: string }[]) {
  const pool = db();
  for (const p of people) {
    await upsertContact(pool, { name: p.name, kind: "grup", role: p.role, phone: p.phone, email: p.email });
  }
}

export async function syncRouteSheetContactsToContacts(contacts: { name: string; role: string; phone: string; company: string }[]) {
  const pool = db();
  for (const c of contacts) {
    await upsertContact(pool, { name: c.name, kind: "ruta", role: c.role, phone: c.phone, company: c.company });
  }
}

export async function syncClientToContacts(clientName: string, details?: { cif?: string; nom?: string; address?: string }) {
  const pool = db();
...
export async function saveContactAction(data: SaveContactInput) {
  ...
  if (data.id) {
    await pool.query(
      `update contacts set name=$1, role=$2, phone=$3, email=$4, company=$5, cif=$6, address=$7, iban=$8, notes=$9 where id=$10`,
      [name, data.role.trim(), data.phone.trim(), data.email.trim(), data.company.trim(), data.cif.trim(), data.address.trim(), data.iban.trim(), data.notes.trim(), data.id]
    );
  } else {
    const id = "ct" + Date.now();
    await pool.query(
      `insert into contacts (id, name, kinds, role, phone, email, company, cif, address, iban, notes)
      ...
      [id, name, data.role.trim(), data.phone.trim(), data.email.trim(), data.company.trim(), data.cif.trim(), data.address.trim(), data.iban.trim(), data.notes.trim()]
    );
  }
  revalidateAll();
}
