...
// tant dins un modal normal (ConcertPosterModal) com incrustat tal qual
// dins d'una targeta (la de "Proper concert" a Inici).
...
export default function ConcertPosterEditor({ concert, band, onClose }: { concert: Concert; band: Band | null; onClose: () => void }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [busy, setBusy] = useState(false);
  const [mapStatus, setMapStatus] = useState("carregant el mapa…");
  // Progrés fictici però lligat a passos reals (mai a un temporitzador):
  // geocodificar (Photon) i demanar els carrers (Overpass) són les dues
  // úniques esperes de veres — cada una fa avançar el percentatge un tram.
  const [mapProgress, setMapProgress] = useState(10);
  ...
  useEffect(() => {
    ...
    async function draw() {
      if (!ctx) return;
      setMapStatus("carregant el mapa…");
      setMapProgress(10);
    ...
    <>
      ...
      <div className="share-month-body">
        ...
        <div className="share-month-controls">
          ...
          {mapStatus && <div className="t-dim" style={{ fontSize: 12 }}>{mapStatus}{mapProgress < 100 ? ` — ${mapProgress}%` : ""}</div>}
          <div>
